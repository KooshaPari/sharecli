...
impl CronField {
    pub fn any() -> Self {
        Self { values: (0..=59).collect(), wildcard: true }
    }
    pub fn single(v: u8) -> Self {
        Self { values: vec![v], wildcard: false }
    }
...
mod tests {
    ...
    fn parse_full() {
        let c = parse("* * * * *").unwrap();
        assert!(c.minute.wildcard);
    }
    #[test]
    fn parse_specific() {
        let c = parse("0 12 * * *").unwrap();
        assert_eq!(c.minute.values, vec![0]);
        assert_eq!(c.hour.values, vec![12]);
    }
    #[test]
    ...
    #[test]
    fn field_matches() {
        assert!(CronField::any().matches(30));
        assert!(CronField::single(5).matches(5));
        assert!(!CronField::single(5).matches(6));
    }
    #[test]
    fn dom_range() {
