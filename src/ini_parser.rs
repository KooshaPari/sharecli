...
pub fn parse(input: &str) -> Result<Vec<Entry>, String> {
    ...
    for (i, raw_line) in input.lines().enumerate() {
        ...
        if let Some(rest) = line.strip_prefix('[') {
            let name = rest
                .strip_suffix(']')
                .ok_or_else(|| format!("line {}: section header missing ']'", i + 1))?;
        ...
        let eq_pos = line.find('=').ok_or_else(|| format!("line {}: missing '='", i + 1))?;
...
mod tests {
    ...
    fn section_and_kv() {
        ...
        assert_eq!(
            v,
            vec![Entry { section: Some("s".into()), key: "foo".into(), value: "bar".into() }]
        );
    }
    #[test]
    ...
    #[test]
    fn quoted_value_with_comment_inside() {
