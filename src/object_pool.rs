...
mod tests {
    ...
    #[test]
    fn put_and_take() {
        let p: ObjectPool<i32> = ObjectPool::new(3);
        p.put(42);
        assert_eq!(p.take(), Some(42));
    }
    #[test]
    fn capacity_cap() {
        let p: ObjectPool<i32> = ObjectPool::new(2);
        assert!(p.put(1));
        assert!(p.put(2));
        assert!(!p.put(3));
    }
    ...
    #[test]
    fn fifo_order() {
        let p: ObjectPool<&str> = ObjectPool::new(3);
        p.put("a");
        p.put("b");
        assert_eq!(p.take(), Some("a"));
        assert_eq!(p.take(), Some("b"));
    }
    #[test]
    fn is_empty() {
        let p: ObjectPool<i32> = ObjectPool::new(3);
