...
mod tests {
    ...
    fn test_extract_id_present() {
        assert_eq!(extract_id(r#"{"id":"abc-123","name":"x"}"#), Some("abc-123".into()));
    }
    #[test]
    fn test_extract_id_missing() {
        assert_eq!(extract_id(r#"{"name":"x"}"#), None);
    }
    #[test]
    fn test_extract_field_present() {
        assert_eq!(extract_field(r#"{"name":"alice"}"#, "name"), Some("alice".into()));
    }
    #[test]
    fn test_extract_field_missing() {
        assert_eq!(extract_field(r#"{"other":1}"#, "name"), None);
    }
    #[test]
    fn test_extract_number() {
        assert_eq!(extract_number(r#"{"count":42}"#, "count"), Some(42));
    }
    #[test]
    fn test_extract_number_missing() {
        assert_eq!(extract_number(r#"{"x":1}"#, "y"), None);
    }
    #[test]
    fn test_extract_number_negative() {
        assert_eq!(extract_number(r#"{"v":-7}"#, "v"), Some(-7));
    }
}
