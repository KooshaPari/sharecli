...
// Minimal MessagePack encoder/decoder (subset: nil, bool, int, uint, float, str, bin, array, map).
pub enum Value {
    Nil,
    Bool(bool),
    Int(i64),
...
pub fn encode(v: &Value, out: &mut Vec<u8>) {
    match v {
        ...
        Value::Array(items) => {
            ...
            } else if items.len() < 65536 {
                out.push(0xdc); out.extend_from_slice(&(items.len() as u16).to_be_bytes());
            } else {
                out.push(0xdd); out.extend_from_slice(&(items.len() as u32).to_be_bytes());
            }
        ...
        Value::Map(entries) => {
            ...
            } else if entries.len() < 65536 {
                out.push(0xde); out.extend_from_slice(&(entries.len() as u16).to_be_bytes());
            } else {
                out.push(0xdf); out.extend_from_slice(&(entries.len() as u32).to_be_bytes());
            }
...
pub fn decode(input: &[u8]) -> Result<(Value, usize), String> {
    ...
    match first {
        ...
        0xcc => {
            if input.len() < 2 { return Err("uint8 too short".into()); }
            Ok((Value::UInt(input[1] as u64), 2))
        }
        0xcd => {
            if input.len() < 3 { return Err("uint16 too short".into()); }
            let u = u16::from_be_bytes([input[1], input[2]]) as u64;
            Ok((Value::UInt(u), 3))
        }
        0xce => {
            if input.len() < 5 { return Err("uint32 too short".into()); }
            let u = u32::from_be_bytes([input[1], input[2], input[3], input[4]]) as u64;
            Ok((Value::UInt(u), 5))
        }
        0xcf => {
        ...
        0xd0 => {
            if input.len() < 2 { return Err("int8 too short".into()); }
            Ok((Value::Int(input[1] as i8 as i64), 2))
        }
        0xd1 => {
            if input.len() < 3 { return Err("int16 too short".into()); }
            let i = i16::from_be_bytes([input[1], input[2]]) as i64;
            Ok((Value::Int(i), 3))
        }
        0xd2 => {
            if input.len() < 5 { return Err("int32 too short".into()); }
            let i = i32::from_be_bytes([input[1], input[2], input[3], input[4]]) as i64;
            Ok((Value::Int(i), 5))
        }
        0xd3 => {
...
mod tests {
    ...
    #[test] fn bin() {
        assert_eq!(roundtrip(&Value::Bin(vec![1, 2, 3, 4, 5])), Value::Bin(vec![1, 2, 3, 4, 5]));
    }
    #[test] fn array() {
        let v = Value::Array(vec![Value::Int(1), Value::Int(2), Value::Int(3)]);
        assert_eq!(roundtrip(&v), v);
    }
    #[test] fn array_nested() {
        let v = Value::Array(vec![Value::Int(1), Value::Array(vec![Value::Int(2)]), Value::Str("x".into())]);
        assert_eq!(roundtrip(&v), v);
    }
    #[test] fn map() {
        let v = Value::Map(vec![(Value::Str("a".into()), Value::Int(1)), (Value::Str("b".into()), Value::Int(2))]);
        assert_eq!(roundtrip(&v), v);
    }
