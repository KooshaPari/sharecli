    ...
    while i < args.len() {
        ...
        if let Some(stripped) = a.strip_prefix("--") {
            if let Some(eq_pos) = stripped.find('=') {
                result.push(Arg {
                    key: stripped[..eq_pos].to_string(),
                    value: Some(stripped[eq_pos + 1..].to_string()),
                });
            } else {
                if i + 1 < args.len() && !args[i + 1].starts_with("--") {
                    result
                        .push(Arg { key: stripped.to_string(), value: Some(args[i + 1].clone()) });
                    i += 1;
                } else {
...
mod tests {
    ...
    fn simple_flag() {
        ...
        let p = parse(&args);
        assert!(has_flag(&p, "verbose"));
    }
    #[test]
    fn key_value_space() {
        let args = vec!["--name".to_string(), "alice".to_string()];
        let p = parse(&args);
        assert_eq!(lookup(&p, "name"), Some("alice"));
    }
    #[test]
    fn key_value_eq() {
        let args = vec!["--name=bob".to_string()];
        let p = parse(&args);
        assert_eq!(lookup(&p, "name"), Some("bob"));
    }
    #[test]
    fn short_flag() {
    ...
    fn positional() {
        ...
        let p = parse(&args);
        assert_eq!(p[0].key, "file.txt");
        assert_eq!(p[0].value, None);
    }
}
