...
pub fn is_ipv6(s: &str) -> bool {
    ...
    let halves: Vec<&str> =
        if double_colon_count == 1 { s.splitn(2, "::").collect() } else { vec![s] };
    let head = halves[0];
    ...
    if double_colon_count == 1 && groups < 8 { /* ok */
    } else if groups != 8 {
        return false;
    }
    let mut total = 0;
    for p in head_parts.iter().chain(tail_parts.iter()) {
        ...
        let n = match u32::from_str_radix(p, 16) {
            Ok(v) => v,
            Err(_) => return false,
        };
        if n > 0xffff {
    ...
    if double_colon_count == 1 && total < 8 {
        true
    } else if total == 8 {
        true
    } else {
