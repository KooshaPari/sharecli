...
mod tests {
    ...
    #[test]
    fn parse_kv() {
        let c = load_from_str("HOST=localhost\nPORT=8080").unwrap();
        assert_eq!(c.get("HOST"), Some("localhost"));
        assert_eq!(c.get("PORT"), Some("8080"));
    }
    #[test]
    fn skip_comments() {
        let c = load_from_str("# comment\nKEY=val").unwrap();
        assert_eq!(c.values.len(), 1);
    }
    #[test]
    fn default_fallback() {
        let c = Config::new();
        assert_eq!(c.get_or("x", "def"), "def");
    }
    #[test]
    fn invalid_line_err() {
        assert!(load_from_str("BADLINE").is_err());
    }
    #[test]
    fn set_get() {
