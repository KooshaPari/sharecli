...
pub struct ZmxCommand { pub program: String, pub args: Vec<String> }
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ZmxCapabilities { pub available: bool, pub durable_pty: bool, pub unix_socket: bool, pub history: bool }

#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ZmxSessionAdapter { binary: String }
...
impl ZmxSessionAdapter {
    ...
    pub fn capabilities(&self, probe_socket: bool) -> ZmxCapabilities {
        ...
        ZmxCapabilities { available, durable_pty: available, unix_socket: available && probe_socket, history: available }
    }
}

fn command_available(binary: &str) -> bool {
    Path::new(binary).is_file() || std::env::var_os("PATH").is_some_and(|path| {
