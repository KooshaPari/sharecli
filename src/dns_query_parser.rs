...
fn read_name(input: &[u8], start: usize) -> Result<(String, usize), String> {
    ...
    loop {
        if cursor >= input.len() {
            return Err(format!(
                "name extends past end of message at byte {}",
                cursor
            ));
        }
        ...
        match len & 0xC0 {
            0x00 => {
                ...
                if l > MAX_LABEL_LENGTH {
                    return Err(format!(
                        "label length {} exceeds 63 at byte {}",
                        l, cursor
                    ));
                }
                cursor += 1;
            ...
            0xC0 => {
                ...
                if cursor + 1 >= input.len() {
                    return Err(format!(
                        "truncated compression pointer at byte {}",
                        cursor
                    ));
                }
            ...
            _ => {
                ...
                return Err(format!(
                    "unsupported label type 0x{:02x} at byte {}",
                    len, cursor
                ));
            }
        }
        ...
        if out.len() > MAX_NAME_LENGTH {
            return Err(format!(
                "name exceeds 255 bytes (truncated at byte {})",
                cursor
            ));
        }
    }
...
mod tests {
    ...
    fn rejects_truncated_question_section() {
        ...
        assert!(
            err.contains("truncated question"),
            "got: {err}"
        );
    }
    ...
    fn rejects_truncated_rdata() {
        ...
        let bytes = vec![
            ...
            0x00, 0x00, 0x01, 0x00, 0x01,
            0x07, 0x65, 0x78, 0x61, 0x6d, 0x70, 0x6c, 0x65, 0x03, 0x63, 0x6f, 0x6d,
            0x00, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x0e, 0x10, 0x00, 0x04,
            0x5d, 0xb8, // only 2 of 4 RDATA bytes
        ];
        let err = parse(&bytes).unwrap_err();
    ...
    fn compression_pointer_resolves_to_question_name() {
        ...
        let bytes = vec![
            0xab, 0xcd, 0x81, 0x80, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
            0x01, 0x61, 0x00, // question: "a." (length at byte 12)
            ...
            0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x3c, 0x00, 0x04, // TYPE/CLASS/TTL/RDLENGTH
    ...
    fn forward_compression_pointer_is_rejected() {
        ...
        let bytes = vec![
            0xab, 0xcd, 0x81, 0x80, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
            0x01, 0x61, 0x00, // question: "a." at byte 12
        ...
        let err = parse(&bytes).unwrap_err();
        assert!(
            err.contains("forward") || err.contains("non-UTF8"),
            "got: {err}"
        );
    }
    ...
    fn name_with_uppercase_is_normalised_to_lowercase() {
        ...
        let bytes = vec![
            0x12, 0x34, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x03, 0x46, 0x4f, 0x4f, 0x02, 0x42, 0x41, 0x00, // "FOO.BA."
            0x00, 0x01, 0x00, 0x01,
        ];
        let pkt = parse(&bytes).unwrap();
    ...
    fn unsupported_extended_label_type_is_rejected() {
        ...
        let bytes = vec![
            0x12, 0x34, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x40, 0x00, 0x00, 0x01, 0x00, 0x01, // extended-label byte + body
        ];
        let err = parse(&bytes).unwrap_err();
    ...
    fn nxdomain_response_with_rcode_3() {
        ...
        let bytes = vec![
            0xbe, 0xef, 0x81, 0x83, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x0c, 0x64, 0x6f, 0x65, 0x73, 0x6e, 0x6f, 0x74, 0x65, 0x78, 0x69, 0x73,
    ...
    fn multiple_questions_and_answers_parsed_in_order() {
        ...
        let bytes = vec![
            // header
            0x00, 0x01, 0x01, 0x00, 0x00, 0x02, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00,
            // Q1
            0x05, b'a', b'l', b'p', b'h', b'a', 0x04, b't', b'e', b's', b't', 0x00,
            0x00, 0x01, 0x00, 0x01,
            // Q2
            0x04, b'b', b'e', b't', b'a', 0x04, b't', b'e', b's', b't', 0x00,
            0x00, 0x01, 0x00, 0x01,
            // A1
            0x05, b'a', b'l', b'p', b'h', b'a', 0x04, b't', b'e', b's', b't', 0x00,
            0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x3c, 0x00, 0x04,
