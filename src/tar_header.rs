...
pub fn parse_header(block: &[u8]) -> Result<Header, String> {
    ...
    let checksum: u32 = u32::from_str_radix(trimmed.trim_start_matches('0'), 8)
        .unwrap_or_else(|_| trimmed.parse().unwrap_or(0));
...
pub fn is_end_of_archive(blocks: &[u8]) -> bool {
    blocks.len() >= 1024
        && blocks[..512].iter().all(|&b| b == 0)
        && blocks[512..1024].iter().all(|&b| b == 0)
}
...
pub fn verify_checksum(block: &[u8]) -> bool {
    ...
    let stored: u32 = match stored_str.and_then(|s| {
        u32::from_str_radix(s.trim_start_matches('0'), 8).ok().or_else(|| s.parse().ok())
    }) {
        Some(v) => v,
        None => return false,
    };
...
fn read_octal(b: &[u8]) -> u64 {
    ...
    u64::from_str_radix(s.trim_start_matches('0').trim(), 8)
