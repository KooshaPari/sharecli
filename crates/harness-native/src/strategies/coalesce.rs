...
fn run_with_hypervisor(
    ...
    let outcome = rt
        .block_on(hv.run(req))
        .map_err(|e| format!("harness coalesce: hypervisor: {e}"))?;
...
mod tests {
    ...
    fn coalesce_strategy_executes_via_hypervisor() {
        ...
        assert_eq!(
            code, 0,
            "AC-008.17: harness coalesce MUST run via Hypervisor::run"
        );
    }
    ...
    fn coalesce_strategy_serves_cache_on_replay() {
        ...
        assert!(
            second.from_cache,
            "AC-008.17: harness coalesce MUST serve cache on replay"
        );
