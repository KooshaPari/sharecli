...
| FR acceptance test files on disk | 6 (`fr001_*`..`fr003_*`) + 1 CLI smoke |
| Integration / cast / coordination test files | 7 |
| Test functions in `tests/` | 67 (`#[test]` / `#[tokio::test]`) |
...
### Critical Gaps
1. **FR-004..FR-005** acceptance files still missing on disk (FR-002/003 Covered).
...
### Immediate Actions
1. Claim `T-220..T-230` in `WORK_DAG.md` to land FR-004..005 acceptance suites.
