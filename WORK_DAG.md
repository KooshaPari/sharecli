...
| T-692 | Dashboard hex drift (token alignment) | FR-003 / C10 L98 | T-685 | M | DONE | Verified `assets/tokens.css` == `src/dashboard.html` `:root` 12/12 bb2 hexes + no hex outside root (`tests/c10_l105_hex_drift.rs` 3/3); `src/theme.rs` mirror verified 2026-08-19 |
