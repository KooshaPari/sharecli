...
fn fr003_l301_fr_grammar_and_acceptance_paths() {
    ...
    let fr_detail = fs::read_to_string(root.join("docs/specs/FR.md"))
        .expect("read docs/specs/FR.md");
    ...
    for (fr_id, paths) in FR_ACCEPTANCE_FILES {
        ...
        for path in *paths {
            assert!(
                ...
                "FUNCTIONAL_REQUIREMENTS must cite {path} for {fr_id}"
            );
            assert!(
...
fn fr003_l303_fr_guardrail_coverage_and_pin() {
    ...
    assert!(
        root
            .join("audit/coverage-snapshots/d3cb7c4.coverage-snapshot.json")
            .is_file(),
        "retained llvm-cov snapshot artifact must exist"
    );
    assert!(
        ci.contains("cargo nextest run"),
        "ci.yml must run nextest guardrail suite"
    );
...
fn fr003_l309_claim_lock_protocol_documented() {
    ...
    assert!(
        agents.contains("Claim-lock protocol"),
        "AGENTS.md must document claim-lock protocol"
    );
    assert!(
        agents.contains("L30.9"),
        "AGENTS.md must cite L30.9 pillar"
    );
    assert!(
        agents.contains("WORK_DAG.md"),
        "AGENTS.md claim-lock must reference WORK_DAG.md"
    );
    assert!(
        agents.contains("Shared path"),
        "AGENTS.md must define shared-path ownership table"
    );
    assert!(
