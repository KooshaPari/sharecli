...
fn zmx_commands_are_shell_free() {
    ...
    assert_eq!(adapter.attach("chat one", &["codex", "resume", "id"]), ZmxCommand::new("zmx", ["attach", "chat one", "codex", "resume", "id"]));
    assert_eq!(adapter.send("chat one", "hello\n"), ZmxCommand::new("zmx", ["send", "chat one", "hello\n"]));
    assert_eq!(adapter.tail("chat one", Some(40)), ZmxCommand::new("zmx", ["tail", "--lines", "40", "chat one"]));
    assert_eq!(adapter.history("chat one", true), ZmxCommand::new("zmx", ["history", "--vt", "chat one"]));
}
