...
fn c07_l70_dev_seed_fixture_valid() {
    ...
    assert!(
        fixture.is_file(),
        "fixtures/dev-seed/config.toml must exist for L70 seed-data"
    );
    ...
    assert!(
        errs.is_empty(),
        "dev seed fixture must pass validate_config: {errs:?}"
    );
    assert!(
...
fn c07_l70_dev_seed_doc_present() {
    let doc = fs::read_to_string(repo_root().join("docs/ops/dev-seed.md")).expect("read dev-seed.md");
    assert!(
        doc.contains("fixtures/dev-seed/config.toml"),
        "docs/ops/dev-seed.md must reference the seed fixture"
    );
    assert!(
        doc.contains("just dev"),
        "docs/ops/dev-seed.md must document just dev entrypoint"
    );
}
