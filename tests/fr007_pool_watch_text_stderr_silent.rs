//! FR-007 — `sharecli pool --watch` text stderr silence (inverse of AC-007.65 JSON)
//! FR: FR-007
//!
//! AC-007.65 `pool --watch` (text mode, no `--json`) MUST NOT print gate or host_watch
...
fn assert_stderr_no_companion_markers(stderr: &str, context: &str) {
    assert!(
        ...
        "{context} stderr MUST NOT include gate companion text (AC-007.65); stderr: {stderr}"
    );
    assert!(
        !stderr.contains(WATCH_MARKER),
        "{context} stderr MUST NOT include host watch companion text (AC-007.65); stderr: {stderr}"
    );
    assert!(
        !stderr.contains("[watch]"),
        "{context} stderr MUST NOT include [watch] footer (AC-007.65); stderr: {stderr}"
    );
}
...
fn assert_gate_before_watch(segment: &str, context: &str) {
    ...
    assert!(
        gate_pos < watch_pos,
        "{context} gate section MUST precede host watch footer (AC-007.65); got: {segment}"
    );
}

fn assert_text_watch_stdout(stdout: &str, context: &str) {
    let frame_count = stdout.matches(POOL_HEADER).count();
    assert!(
        frame_count >= 2,
    ...
    assert!(
        ...
        "{context} stdout MUST include gate section (AC-007.65); got: {stdout}"
    );
    assert!(
        stdout.contains(WATCH_MARKER),
        "{context} stdout MUST include host watch section (AC-007.65); got: {stdout}"
    );
    assert!(
        stdout.contains("[watch]"),
        "{context} stdout MUST include [watch] footer (AC-007.65); got: {stdout}"
    );
    for (idx, segment) in stdout.split(POOL_HEADER).skip(1).enumerate() {
...
/// FR-007 / AC-007.65 — pool --watch text keeps stderr silent across refresh cycles.
#[test]
#[serial_test::serial]
fn fr007_pool_watch_text_stderr_silent() {
    let mut child = bin()
        .args(["pool", "--watch", "1"])
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
    ...
    assert_text_watch_stdout(&stdout, "pool --watch");
}
