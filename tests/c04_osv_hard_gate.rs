...
fn fr003_ci_yml_osv_hard_gate_wired() {
    ...
    assert!(
        !ci.contains("Soft gate (always pass)"),
        "ci.yml must not contain OSV soft pass shim"
    );
    ...
    assert!(
        needs_line.contains("osv"),
        "ci-success must need osv job; got: {needs_line}"
    );
}
...
fn fr003_osv_yml_no_soft_shim() {
    let osv = fs::read_to_string(repo_root().join(".github/workflows/osv.yml"))
        .expect("read osv.yml");

    assert!(
        !osv.contains("Soft gate (always pass)"),
        "osv.yml must not contain soft pass shim"
    );
    assert!(
        osv.contains("--severity=HIGH,CRITICAL"),
        "osv.yml must scan HIGH/CRITICAL severity"
    );

    let scan_section = osv
        .split("Scan Cargo.lock with OSV-Scanner")
        .nth(1)
        .expect("osv.yml scan step");
    let scan_block = scan_section
        .split("Upload SARIF")
        .next()
        .expect("scan block before SARIF");
